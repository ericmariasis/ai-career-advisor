...
router.get('/:id', async (req, res) => {
    try {
        ...
        const jobData = await r.json.get(jobKey, { path: '.' });
        if (!jobData) {
 ...
 */
function extractAISkills(jobData) {
    ...
    return aiSkills.filter(skill => !baseSkills.some((baseSkill) => baseSkill.toLowerCase() === skill.toLowerCase()));
}
/**
 * Determine seniority level from job data
 * In production, this would use ML classification
 */
function determineSeniority(jobData) {
    const title = (jobData.title || '').toLowerCase();
    const salary = jobData.salary_estimate || 0;
