...
// ── 1. connect ───────────────────────────────────────────────────────────────
const client = (0, redis_1.createClient)({ url: process.env.REDIS_URL || 'redis://redis:6379' });
async function ensureJobsIndex() {
    try {
        ...
        if (!indices.includes('jobsIdx')) {
            ...
            await client.ft.create('jobsIdx', {
                '$.title': {
                    type: 'TEXT',
                    AS: 'title',
                    sortable: true
        ...
        const seedPath = (0, path_1.resolve)(process.cwd(), 'jobs_seed.json');
        console.log(`📂 Reading seed file: ${seedPath}`);
        const raw = (0, fs_1.readFileSync)(seedPath, 'utf8');
